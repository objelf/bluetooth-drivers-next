...
int btmtk_usb_setup(struct hci_dev *hdev, struct btmtk_data_i *i)
{
	...
	data->dev_id = dev_id;
	data->reset_sync = i->reset_sync;
	data->udev = i->udev;
	data->intf = i->intf;
	data->ctrl_anchor = i->ctrl_anchor;

	err = btmtk_register_coredump(hdev, i->drv_name, fw_version);
	if (err < 0)
	...
	case 0x7925:
		...
		if (err < 0) {
			...
			clear_bit(BTMTK_FIRMWARE_LOADED, &data->flags);
			return err;
		}
...
int btmtk_usb_recv_acl_mtk(struct hci_dev *hdev, struct sk_buff *skb)
{
	struct btmtk_data *data = hci_get_priv(hdev);
...
EXPORT_SYMBOL_GPL(btmtk_usb_recv_acl_mtk);

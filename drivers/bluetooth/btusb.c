...
{
	struct btusb_data *data = hci_get_drvdata(hdev);
	struct btmtk_data *mdata = hci_get_priv(hdev);
	int err;

	/* It's MediaTek specific bluetooth reset mechanism via USB */
	if (test_and_set_bit(BTMTK_HW_RESET_ACTIVE, &mdata->flags)) {
		bt_dev_err(hdev, "last reset failed? Not resetting again");
		return -EBUSY;
	}
	...
	err = btmtk_usb_subsys_reset(hdev, mdata->dev_id);

	usb_queue_reset_device(data->intf);
	clear_bit(BTMTK_HW_RESET_ACTIVE, &mdata->flags);
...
{
	struct btusb_data *data = hci_get_drvdata(hdev);
	struct btmtk_data_i *i = hci_get_priv(hdev);

	i->reset_sync = btusb_mtk_reset;
	i->drv_name = btusb_driver.name;

	/* MediaTek WMT vendor cmd requiring below USB resources to
	 * complete the handshaket.
	 * */
	i->intf = data->intf;
	i->udev = data->udev;
	i->ctrl_anchor = &data->ctrl_anchor;

	return btmtk_usb_setup(hdev, i);
}
		       ...
		       const struct usb_device_id *id)
{
	...
	if (IS_ENABLED(CONFIG_BT_HCIBTUSB_MTK) &&
		...
		hdev->shutdown = btmtk_usb_shutdown;
		hdev->manufacturer = 70;
		hdev->cmd_timeout = btmtk_reset_sync;
		hdev->set_bdaddr = btmtk_set_bdaddr;
		set_bit(HCI_QUIRK_BROKEN_ENHANCED_SETUP_SYNC_CONN, &hdev->quirks);
		set_bit(HCI_QUIRK_NON_PERSISTENT_SETUP, &hdev->quirks);
		data->recv_acl = btmtk_usb_recv_acl_mtk;
	}
